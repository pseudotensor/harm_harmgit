...
// from init.nsbh.c
extern int bound_prim_user_dir_nsbh(int boundstage, SFTYPE boundtime, int whichdir, int boundvartype, FTYPE (*prim)[NSTORE2][NSTORE3][NPR]);
extern int bound_pstag_user_dir_nsbh(int boundstage, SFTYPE boundtime, int whichdir, int boundvartype, FTYPE (*prim)[NSTORE2][NSTORE3][NPR]);
extern int bound_emf_user_nsbh(int boundstage, SFTYPE boundtime, int whichdir, int boundvartype, FTYPE (*prim)[NSTORE2][NSTORE3][NPR]);
...
int bound_prim_user_dir(int boundstage, SFTYPE boundtime, int whichdir, int boundvartype, FTYPE (*prim)[NSTORE2][NSTORE3][NPR])
{
  ...
  // Boundary conditions for NS (comes after normal BCs in case NS near BC -- shouldn't be!, since want NS conditiosn to dominate)
  bound_prim_user_dir_nsbh(boundstage,boundtime,whichdir,boundvartype,prim);
...
int bound_pstag_user_dir(int boundstage, SFTYPE boundtime, int whichdir, int boundvartype, FTYPE (*prim)[NSTORE2][NSTORE3][NPR])
{
  ...
  // Boundary conditions for NS
  bound_pstag_user_dir_nsbh(boundstage,boundtime,whichdir,boundvartype,prim);
               ...
               FTYPE *p2interp_l, FTYPE *p2interp_r )
{
  ...
  // remap for NS surface
  remapplpr_nsbh(dir,idel,jdel,kdel,i,j,k, p2interp, dq, pleft, pright, p2interp_l, p2interp_r);
