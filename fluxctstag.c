...
#pragma omp for schedule(OPENMPSCHEDULE(),OPENMPCHUNKSIZE(blocksize)) nowait
	...
	//	if(i==0 && k==0 && (j==0 || j==N2)) dualfprintf(fail_file,"vpot2field2: j=%d B2s=%21.15g : gdet=%21.15g p1=%21.15g p1=%21.15g\n",j,MACP0A1(pfield,i,j,k,B1-1+dir),igdetgnosing[dir],+(NOAVGCORN_2(A[1],i,j,kp1mac(k))-NOAVGCORN_2(A[1],i,j,k))/(dx[3]),-(NOAVGCORN_2(A[3],ip1mac(i),j,k)-NOAVGCORN_2(A[3],i,j,k))/(dx[1]));
	
	//	if(i==0 && k==0 && (j==0 || j==N2)){
	//	if((i>=-2 && i<=2) && k==0 && (j==0 || j==N2)){
	//	  dualfprintf(fail_file,"i=%d j=%d A3ip1=%21.15g A3i=%21.15g\n",i,j,NOAVGCORN_2(A[3],ip1mac(i),j,k),NOAVGCORN_2(A[3],i,j,k));
	//	}
...
#define BRFACEINTERP 1 // only used if Nvec[dir]==1
...
#pragma omp for schedule(OPENMPSCHEDULE(),OPENMPCHUNKSIZE(blocksize)) ///nowait // can't use "nowait" when using p2interp for each dir in next loop
	...
	MACP0A1(p2interp,i,j,k,BFACEINTERP) = prface_l[B1-1+dir];
#endif
...
#pragma omp for schedule(OPENMPSCHEDULE(),OPENMPCHUNKSIZE(blocksize)) //nowait // Don't wait since each direction is independent // NO: use of pleft, pright, and some use of p2interp (that depend on each dir) from loop above makes not possible to use "nowait"
	OPENMP3DLOOPBLOCK{
	  ...
	  MACP1A3(pvbcorn,edgedir,i,j,k,interpdir,Aodir1,Aodir2) = p2interp_l[npr2interplist[1]]; // current p_l for previous p_l 
	  MACP1A3(pvbcorn,edgedir,i,j,k,interpdir,Bodir1,Bodir2) = p2interp_r[npr2interplist[1]]; // current p_r for previous p_l
	  MACP1A3(pvbcorn,edgedir,i,j,k,interpdir,Codir1,Codir2) = p2interp_l[npr2interplist[2]]; // current p_l for previous p_r
	  MACP1A3(pvbcorn,edgedir,i,j,k,interpdir,Dodir1,Dodir2) = p2interp_r[npr2interplist[2]]; // current p_r for previous p_r
	}// endCOMPZSLOOP
