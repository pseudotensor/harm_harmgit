...
int Utoprim_2d(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
  ...
  if(pflag[icurr][jcurr][FLAGUTOPRIMFAIL]&&(debugfail>=1)) dualfprintf(fail_file, "utoprim_2d(): Failed to find a prim. var. solution!! t=%21.15g nstep=%ld i=%d j=%d\n",t,nstep,icurr,jcurr);
...
static int Utoprim_new_body(FTYPE U[NPR], struct of_geom *ptrgeom,  FTYPE prim[NPR])
{
  ...
  static int find_root_2D_gen(double x0, double *xnew) ;
  ...
  */
  ...
  while( (( W_last*W_last*W_last * ( W_last + 2.*Bsq ) - QdotBsq*(2.*W_last + Bsq) ) <= W_last*W_last*(Qtsq-Bsq*Bsq))
    ...
    i_increase++;
#if(!OPTIMIZED)
    dualfprintf(fail_file,"badval :  W = %26.20e, i_increase = %d \n", W_last, i_increase); 
#endif
  }
#if(!OPTIMIZED)
  if( i_increase >= 10 ) { 
  ...
  if( nstep == 3 ) { 
    if( ((icurr==89) && (jcurr==88||jcurr==111)) || (icurr==92&&(jcurr==86||jcurr==113)) 
	|| (icurr==92&&(jcurr==86||jcurr==113) ) || (icurr==94&&(jcurr==84||jcurr==115) ) 
	|| (icurr==105&&(jcurr==84||jcurr==115) ) || (icurr==107&&(jcurr==86||jcurr==113) ) 
	|| (icurr==110&&(jcurr==88||jcurr==111) ) ) {
      dualfprintf(fail_file, "\n <--------- %26.20e %26.20e %26.20e %26.20e %26.20e  \n", Bsq,QdotBsq,Qdotn,D,Qtsq);
    }
  }
...
static int find_root_2D_gen(double x0, double *xnew) 
{
  ...
  static  void func_vsq2(   double [], double [], double [], double [][NEWT_DIM], double *f, double *df, int n);
  static  double  res_sq_vsq( double [] );
  static  double  res_sq_vsq2( double [] );
  static double x1_of_x0(double x0 ) ;
  static int general_newton_raphson( double x[], int n, int do_line_search,
				     void (*funcd) (double [], double [], double [], double [][NEWT_DIM], 
						    double *, double *, int), 
...
#if(!OPTIMIZED)
  if( ltrace ) {
    dualfprintf(fail_file, "find_root_2D_gen():  x[0] = %26.20e , x[1] = %26.20e \n", x[0], x[1]);
...
#if(!OPTIMIZED)
    if( ltrace ) {
      dualfprintf(fail_file, "find_root_2D_gen():  ntries, x[0,1] = %4i  %26.20e   %26.20e  \n", ntries, x[0], x[1]);
  ...
  if( (ntries > MAX_NEWT_RETRIES) &&  (retval==1)  ) {
    if( debugfail >= 2 ) { 
      ...
      dualfprintf(fail_file, "find_root_2D_gen():  ntries = %d , x[0] = %26.20e , x[1] = %26.20e  \n", ntries, x[0], x[1]); 
    }
  }
#endif
...
static void func_vsq(double x[], double dx[], double resid[], double jac[][NEWT_DIM], double *f, double *df, int n)
{
...
#if(!OPTIMIZED)
  if( ltrace ) {
    dualfprintf(fail_file,"func_vsq(): x[0] = %26.20e , x[1] = %26.20e , dx[0] = %26.20e , dx[1] = %26.20e , f = %26.20e \n", 
	    x[0], x[1],dx[0], dx[1], *f);
...
static void func_vsq2(double x[], double dx[], double resid[], double jac[][NEWT_DIM], double *f, double *df, int n)
{
...
#if(!OPTIMIZED)
  if( ltrace ) {
    dualfprintf(fail_file,"func_vsq2(): x[0] = %26.20e , x[1] = %26.20e , dx[0] = %26.20e , dx[1] = %26.20e , f = %26.20e \n", 
	    x[0], x[1],dx[0], dx[1], *f);
...
/*********************************************************
**********************************************************/
static double res_sq_vsq(double x[])
{
...
#if(!OPTIMIZED)
  if( ltrace  ) {
    dualfprintf(fail_file,"res_sq_vsq(): W,vsq,resid0,resid1,f =   %26.20e  %26.20e  %26.20e  %26.20e  %26.20e  \n",
	    W,vsq,resid[0],resid[1],tmp3);
    dualfprintf(fail_file,"res_sq_vsq(): Qtsq, Bsq, QdotBsq, Qdotn =   %26.20e  %26.20e  %26.20e  %26.20e  \n",
...
/*********************************************************
**********************************************************/
static double res_sq_vsq2(double x[])
{
  ...
  for (;;) {
    ...
    *f=(*func)(x+1);
    ...
    if( bad_step ) { 
      *check = 2;
#if(!OPTIMIZED)
      dualfprintf(fail_file,"my_lnsrch(): bad_step = 1,  f = %26.20e , alam = %26.20e \n", *f, alam); 
      for( i = 1; i <= n; i++ ) { 
	dualfprintf(fail_file,"my_lnsrch(): (xold[%d], x[%d], p[%d]) = %26.20e , %26.20e , %26.20e \n", i,i,i, xold[i],x[i],p[i]);
      }
#endif
      return;
    }
...
#if(!OPTIMIZED)
	    if( disc < -1.e-10 ) {
	      if( ltrace ) dualfprintf(fail_file,"my_lnsrch(): Big Roundoff problem:  disc = %26.20e \n", disc);
	    }
#endif	      
	    disc = 0.;
	  }
