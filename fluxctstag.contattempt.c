...
void slope_lim_continuous_e2z(int realisinterp, int dir, int idel, int jdel, int kdel, FTYPE (*primreal)[NSTORE2][NSTORE3][NPR], FTYPE (*p2interp)[NSTORE2][NSTORE3][NPR2INTERP], FTYPE (*dq)[NSTORE2][NSTORE3][NPR2INTERP], FTYPE (*pleft)[NSTORE2][NSTORE3][NPR2INTERP], FTYPE (*pright)[NSTORE2][NSTORE3][NPR2INTERP], struct of_loop *face2centloop)
{
  ...
  PINTERPLOOP(pliter,pl){ // probably only pl=B1+dir-1;
    ...
    locallimgen=choose_limiter(dir, 0,0,0,pl);
    usedqgen =MAX(usedqgen, (locallimgen<PARA)&&(LIMADJUST==0));
  }
...
#pragma omp for schedule(OPENMPSCHEDULE(),OPENMPCHUNKSIZE(blocksize)) ///nowait // can't use "nowait" when using p2interp for each dir in next loop
    OPENMP3DLOOPBLOCK{
      ...
      PINTERPLOOP(pliter,pl){ // probably only pl=B1+dir-1;
        // difference oriented on difference in dir
        // modify p2interp in place by starting on lower part and working up, i.e. dBc(i) = Bf(i+1)-Bf(i) and no longer need Bf(i) for i-direction.
        // OPENMPNOTE: Can't replace p2interp in place because use different cells for each i,j,k
        GLOBALMACP0A1(ptemparray,i,j,k,pl) = MACP0A1(p2interp,i+SHIFT1*(dir==1),j+SHIFT2*(dir==2),k+SHIFT3*(dir==3),pl) - MACP0A1(p2interp,i,j,k,pl) ;
      }
    }// end LOOP
  }
  }
  ...
  if( LINEINTERPTYPE(lim[dir]) ){ // this overrides lim, but lim must still be set properly
    ...
    // GODMARK: set_interppoint() inside this function sets starting and ending position for loops, and as set c2e always needs more than e2c for obtaining flux, so leaving as for c2e is fine for now
    slope_lim_linetype_c2e(realisinterp, ENOPRIMITIVE, ENOINTERPTYPE, dir, idel, jdel, kdel, primreal, NULL, GLOBALPOINT(ptemparray), pleft, pright);
      
    // ENOMARK: for ENO should really  use special _e2c_cont() function that assumes continuity is required GODMARK
    //    slope_lim_linetype_e2c_cont(realisinterp, ENOPRIMITIVE, ENOINTERPTYPE, dir, idel, jdel, kdel, primreal, NULL, ptemparray, pcent);
  }
  else{
    // Should really interpolate such that continuous GODMARK
    PINTERPLOOP(pliter,pl){
      slope_lim_pointtype(ENOINTERPTYPE, realisinterp, pl, dir, idel, jdel, kdel, primreal, GLOBALPOINT(ptemparray), dq, pleft, pright); // GODMARK: overwritting dq from other type of interpolation
    }
  }
  ...
  if(usedqgen==1){
    ...
    // get loop details
    //  int idel = fluxloop[dir][FIDEL];
    //  int jdel = fluxloop[dir][FJDEL];
    //  int kdel = fluxloop[dir][FKDEL];
...
#pragma omp for schedule(OPENMPSCHEDULE(),OPENMPCHUNKSIZE(blocksize)) ///nowait // can't use "nowait" when using p2interp for each dir in next loop
    OPENMP3DLOOPBLOCK{
      ...
      PINTERPLOOP(pliter,pl){ // probably only pl=B1+dir-1;
        ...
        // difference oriented on difference in dir
        // modify p2interp in place by starting on lower part and working up, i.e. dBc(i) = Bf(i+1)-Bf(i) and no longer need Bf(i) for i-direction.
        //      MACP0A1(p2interp,i,j,k,pl) = MACP0A1(p2interp,i+SHIFT1*(dir==1),j+SHIFT2*(dir==2),k+SHIFT3*(dir==3),pl) - MACP0A1(p2interp,i,j,k,pl) ;

        //GLOBALMACP0A1(ptemparrayalt,i,j,k,pl) = MACP0A1(p2interp,i,j,k,pl) + (1.0/3.0)*GLOBALMACP0A1(ptemparray,i,j,k,pl) + (5.0/24.0)*MACP0A1(pleft,i,j,k,pl) - (1.0/24.0)*MACP0A1(pright,i,j,k,pl);
        //GLOBALMACP0A1(ptemparrayalt,i,j,k,pl) = MACP0A1(p2interp,i,j,k,pl) + (3.0/8.0)*MACP0A1(pleft,i,j,k,pl) + (1.0/8.0)*MACP0A1(pright,i,j,k,pl);

        GLOBALMACP0A1(ptemparrayalt,i,j,k,pl) = (3.0/4.0)*MACP0A1(p2interp,i,j,k,pl) + (1.0/4.0)*MACP0A1(p2interp,i+SHIFT1*(dir==1),j+SHIFT2*(dir==2),k+SHIFT3*(dir==3),pl) + (1.0/4.0)*MACP0A1(pleft,i,j,k,pl);
...
#pragma omp for schedule(OPENMPSCHEDULE(),OPENMPCHUNKSIZE(blocksize)) ///nowait // can't use "nowait" when using p2interp for each dir in next loop
    OPENMP3DLOOPBLOCK{
      ...
      PINTERPLOOP(pliter,pl){ // probably only pl=B1+dir-1;
        ...
        int locallim=choose_limiter(dir, 0,0,0,pl);
        int usedq = (locallim<PARA)&&(LIMADJUST==0);

        // now fake-assign to pleft,pright, or dq
        if(usedq){
          MACP0A1(dq,i,j,k,pl) = 2.0*(GLOBALMACP0A1(ptemparrayalt,i,j,k,pl) - MACP0A1(p2interp,i,j,k,pl));
          MACP0A1(dq,i + idel,j + jdel,k + kdel,pl) = 2.0*(-GLOBALMACP0A1(ptemparrayalt,i,j,k,pl) + MACP0A1(p2interp,i + idel,j + jdel,k + kdel,pl));
          ...
          MACP0A1(pright,i,j,k,pl) = MACP0A1(pleft,i+idel,j+jdel,k+kdel,pl) = GLOBALMACP0A1(ptemparrayalt,i,j,k,pl);
        }
      }
    } // END LOOP
  }
