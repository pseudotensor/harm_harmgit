...
static int Utoprimgen_failwrapper(int showmessages, int allowlocalfailurefixandnoreport, int finalstep, int evolvetype, int inputtype,FTYPE *U,  struct of_geom *ptrgeom, FTYPE *pr, struct of_newtonstats *newtonstats)
{
  ...
  int eomtype=3;
  struct of_state *qptr=NULL;
  MYFUN(Utoprimgen(showmessages, allowlocalfailurefixandnoreport, finalstep, &eomtype, 1, -1, evolvetype, inputtype, U, qptr, ptrgeom, -1.0, pr, pr, newtonstats),"phys.tools.rad.c:Utoprimgen_failwrapper()", "Utoprimgen", 1);
...
int mathematica_report_check(int failtype, long long int failnum, int gotfirstnofail, FTYPE realdt,struct of_geom *ptrgeom, FTYPE *pinuse, FTYPE *pin, FTYPE *uu0, FTYPE *uu, FTYPE *Uiin, FTYPE *Ufin, FTYPE *CUf, struct of_state *q, FTYPE *dUother)
{
  ...
  if(0){ // old mathematica style
    ...
    // 134 things
    dualfprintf(fail_file,"\nFAILINFO: %d %d %lld %d %26.20g ",failtype,myid,failnum,gotfirstnofail,realdt);
                           ...
                           FTYPE *pin, FTYPE *Uiin, FTYPE *Ufin, FTYPE *CUf, struct of_geom *ptrgeom, struct of_state *q, FTYPE *dUother, FTYPE (*dUcomp)[NPR])
{
  ...
  if(GETADVANCEDUNEW0FOREXPLICIT){
    ...
    while(1){ // loop bounded by fracdtuu0 becoming too small
      ...
      // initialize counters
      newtonstats.nstroke=newtonstats.lntries=0;
      dualfprintf(fail_file,"BEFORE 2\n");
  ...
  while(1){
    ...
    // get prnew(Unew)
    newtonstats.nstroke=newtonstats.lntries=0;
    dualfprintf(fail_file,"BEFORE 3\n");
    int failutoprim=Utoprimgen_failwrapper(showmessages, allowlocalfailurefixandnoreport, finalstep, EVOLVEUTOPRIM, UNOTHING, Unew, ptrgeom, prnew, &newtonstats);
...
int koral_source_rad(int whichradsourcemethod, FTYPE *pin, FTYPE *pb, FTYPE *pf, int *didreturnpf, int *eomtype, FTYPE *Uiin, FTYPE *Ufin, FTYPE *CUf, struct of_geom *ptrgeom, struct of_state *q, FTYPE dissmeasure, FTYPE *dUother, FTYPE (*dUcomp)[NPR])
{
  int pliter,pl;
  ...
  FTYPE pinorig[NPR],Uiinorig[NPR],Ufinorig[NPR],pborig[NPR];
  ...
  *qorig=*q;
  ...
  /////////////////
    ...
    if(failimplicit){
      if(IMPLICITREVERTEXPLICIT){ // single level recusive call (to avoid duplicate confusing code)
        ...
        int failexplicit=koral_source_rad(SOURCEMETHODEXPLICITSUBCYCLEREVERSIONFROMIMPLICIT, pinorig, pborig, pf, didreturnpf, eomtype, Uiinorig, Ufinorig, CUf, ptrgeom, qorig, dissmeasure, dUother, dUcomp);
        if(failexplicit==EXPLICITFAILED){
  ...
  /////////////////
    ...
    int failreturn=koral_source_rad(SOURCEMETHODEXPLICITCHECKSFROMIMPLICIT, pinorig, pborig, pf, didreturnpf, eomtype, Uiinorig, Ufinorig, CUf, ptrgeom, qorig, dissmeasure, dUother, dUcomp);
    ...
    if(doimplicit){
      ...
      return(koral_source_rad(SOURCEMETHODIMPLICIT, pinorig, pborig, pf, didreturnpf, eomtype, Uiinorig, Ufinorig, CUf, ptrgeom, qorig, dissmeasure, dUother, dUcomp));
    }// end if doimplicit==1
...
void mhd_calc_rad(FTYPE *pr, int dir, struct of_geom *ptrgeom, struct of_state *q, FTYPE *radstressdir, FTYPE *blob)
{
  int jj;
...
static int get_m1closure_gammarel2(int showmessages, struct of_geom *ptrgeom, FTYPE *Avcon, FTYPE *Avcov, FTYPE *gammarel2return, FTYPE *deltareturn, FTYPE *numeratorreturn, FTYPE *divisorreturn)
{
  ...
  //  dualfprintf(fail_file,"gammarel2=%g gamma2=%g\n",gammarel2,gamma2);
