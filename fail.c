...
int fail(int i, int j, int k, int loc, int f, int fail_type)
{

  // failed use for diag below to avoid bad calculations
  if(f==-1){
    return(1);    // then just test and return(1) sufficient (i.e. no debug info)
  }
  else{
    ...
    // do nothing since never will fail since if failed, doesn't do fail-related function calls
