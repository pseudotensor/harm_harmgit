...
void set_arrays_multidimen()
{
  ...
  FULLLOOP for(l=1;l<=COMPDIM;l++){
    GLOBALMACP0A1(dtijk,i,j,k,l) = -1.0; //valueinit; MAVARACHANGE in order to get dt to not drop to 1e-300 on first tstep when percelldt==1
  }
#endif
  ...
  GLOBALPOINT(Vstore) = (FTYPE PTRMETMACP1A1(Vstore,FILL,N1M+SHIFT1*3,N2M+SHIFT2*3,N3M+SHIFT3*3,NDIM)) (&(BASEMETMACP1A1(Vstore,0,N1BND+SHIFT1,N2BND+SHIFT2,N3BND+SHIFT3,0)));
#endif
