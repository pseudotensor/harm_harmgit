...
if [ $# -ne $EXPECTED_ARGS ]
then
  echo "Usage: `basename $0` {modjob} {submitjob} {whichsystem} {basejob} {basesim} {old ext} {new ext}"
  echo "E.g. `basename $0` 1 1 1 td7 thickdisk7 nextnexte nextnextf"
  echo "E.g. `basename $0` 1 1 2 t3 thickdisk3 r s"
  echo "E.g. `basename $0` 1 1 2 rad1 rad1 a b"
  exit
fi
...
oldname=$basesim$oldext
newname=$basesim$newext
oldjob=$basejob$oldext
newjob=$basejob$newext
...
cp $WORKDIR/$oldname/coordparms.dat $WORKDIR/$oldname/dimensions.txt $WORKDIR/$newname/
...
then
    ...
    alias mv='mv'
    alias cp='cp'
    mv batch.${subtype}.$batchpart.$basesim batch.${subtype}.$batchpart.$basesim.orig
    sed 's/'$oldname'/'$newname'/g' batch.${subtype}.$batchpart.$basesim.orig > batch.${subtype}.$batchpart.$basesim.1
    sed 's/'$oldjob'/'$newjob'/g' batch.${subtype}.$batchpart.$basesim.1 > batch.${subtype}.$batchpart.$basesim.2

    if [ $whichsystem -eq 1 ]
    then
	sed 's/$WORK/$WORKDIR/g' batch.${subtype}.$batchpart.$basesim.2 > batch.${subtype}.$batchpart.$basesim.3
	sed 's/ibrun .\/$FILENAME $NCPUX1 $NCPUX2 $NCPUX3 > $FILENAME.so/ibrun .\/$FILENAME $NCPUX1 $NCPUX2 $NCPUX3 1 0 > $FILENAME.so/g' batch.${subtype}.$batchpart.$basesim.3 > batch.${subtype}.$batchpart.$basesim.4
    fi

    if [ $whichsystem -eq 2 ]
    then
	sed 's/$SCRATCH/$WORKDIR/g' batch.${subtype}.$batchpart.$basesim.2 > batch.${subtype}.$batchpart.$basesim.3
	sed 's/aprun -n $NTOT $RUNDIR\/$FILENAME $NCPUX1 $NCPUX2 $NCPUX3 *$/aprun -n $NTOT $RUNDIR\/$FILENAME $NCPUX1 $NCPUX2 $NCPUX3 1 0/g' batch.${subtype}.$batchpart.$basesim.3 > batch.${subtype}.$batchpart.$basesim.4
    fi

    if [ $whichsystem -eq 3 ]
    then
	sed 's/$SCRATCH/$WORKDIR/g' batch.${subtype}.$batchpart.$basesim.2 > batch.${subtype}.$batchpart.$basesim.3
	sed 's/mpiexec -np $NTOT $RUNDIR\/$FILENAME $NCPUX1 $NCPUX2 $NCPUX3 *$/mpiexec -np $NTOT $RUNDIR\/$FILENAME $NCPUX1 $NCPUX2 $NCPUX3 1 0 > output.txt/g' batch.${subtype}.$batchpart.$basesim.3 > batch.${subtype}.$batchpart.$basesim.4
    fi
    ...
    mv batch.${subtype}.$batchpart.$basesim.4 batch.${subtype}.$batchpart.$basesim
    rm -rf batch.${subtype}.$batchpart.$basesim.1 batch.${subtype}.$batchpart.$basesim.2  batch.${subtype}.$batchpart.$basesim.3  batch.${subtype}.$batchpart.$basesim.4
fi
...
then
    ...
    ${subtype} < batch.${subtype}.$batchpart.$basesim
fi
